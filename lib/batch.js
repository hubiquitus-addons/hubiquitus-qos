...
function loop() {
  setInterval(function () {
    ...
    var query = {
      date: {'$lt': (Date.now() - conf.timeout)}, err: {'$exists': false}
    };
    var options = {
      sort: [['retry', 'asc'], ['date', 'asc']]
    };

    var cursor = db.collection(conf.mongo.collection).find(query, options);
...
function replay(item) {
  ...
  hubiquitus.send(req.from, hubiquitus.utils.aid.bare(req.to), req.content, properties.processingTimeout, function (err) {
    if (!err) {
      ...
      logger.error('failed to replay req, remains in queue', {err: err, item: item});
      item.retry++;
      db.collection(conf.mongo.collection).save(item, function (err) {
        if (err) {
          logger.warn('failed to update retry field of item', {err: err, item: item});
        }
      });
    }
  }, {qos: true, qos_retry: item.retry});
}
