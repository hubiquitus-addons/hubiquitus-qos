  ...
  (function tick() {
    ...
    stream.on('data', function (item) {
      ...
      hubiquitus.monitoring.pingContainer(item.container.ID, function (err) {
        if (err && err.code === 'TIMEOUT' || (now - item.date) > 12 * 3600 * 1000) {
          if (err && err.code === 'TIMEOUT') {
            logger.trace('handler\'s container dead; request replay', {item: item});
          } else {
            logger.trace('Message too old; request replay', {item: item});
          }
...
function replay(item, done) {
  ...
  hubiquitus.send(req.from, to, req.content, properties.gcReplayTimeout, function (err) {
    if (!err) {
      db.collection(properties.collection).updateOne({_id: item._id}, {'$set': {removalTime: new Date()}}, function (err) {
        if (err) {
      ...
      db.collection(properties.collection).updateOne({_id: item._id}, {$inc: {retry: 1}}, function (err) {
        if (err) {
