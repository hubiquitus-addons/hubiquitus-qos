...
function loop() {
  (function tick() {
    ...
    var query = {date: {'$lt': (Date.now() - properties.gcTimeout)}, err: {'$exists': false}};
    var options = {sort: [['retry', 'asc'], ['date', 'asc']]};
    var stream = db.collection(properties.mongo.collection)
      .find(query, options)
...
function replay(item, done) {
  ...
  hubiquitus.send(req.from, to, req.content, properties.gcReplayTimeout, function (err) {
    if (!err) {
      db.collection(properties.mongo.collection).remove({_id: item._id}, function (err) {
        if (err) {
      ...
      db.collection(properties.mongo.collection).update({_id: item._id}, {$inc: {retry: 1}}, function (err) {
        if (err) {
  ...
  }, {qos: true, qos_id: hubiquitus.utils.uuid(), qos_retry: item.retry + 1});
}
